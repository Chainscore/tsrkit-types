...
#Removed in heritance from int
class Int( Codable, metaclass=IntCheckMeta):
    """
    ...
    #     if self.byte_size > 0:
    #         max_v = (self._bound // 2 if self.signed else self._bound) - 1  
    #         min_v = -1 * self._bound // 2 if self.signed else 0
    #     else:
    #         min_v = -1 * self._bound // 2 if self.signed else 0
    #         max_v = (self._bound // 2 if self.signed else self._bound) - 1  
        
    #     if not (min_v <= value <= max_v):
    #         raise ValueError(f"Int: {self.__class__.__name__} out of range: {value!r} "
    #                         f"not in [{min_v}, {max_v}]")

    #     self._val = self._ctype(value)
    ...
    #     byte_size = size // 8 if size else 8
    #     ctype = _ctypes_map.get((byte_size, signed), c_uint64)
    ...
    byte_size: int = 0
    signed = False
    _ctype = c_uint64  # Default general

    def __init__(self, value: Any):
        self._val = self._ctype(int(value))

    def __int__(self):
        return self._val.value

    def __repr__(self):
        return f"{self.__class__.__name__}({self._val.value})"

    def _wrap_op(self, other: Any, op: Callable[[int, int], int]):
        return type(self)(op(self._val.value, int(other)))
    ...
    def __class_getitem__(cls, data: Optional[Union[int, tuple, bool]]):
        ...
        byte_size = size // 8 if size else 8
        ctype = _ctypes_map.get((byte_size, signed), c_uint64)
        name = f"{'I' if signed else 'U'}{size or 64}"
    ...
    def to_unsigned(self) -> "Int":
        ...
        return type(self)(int(self) + (self._bound // 2))

    def encode_size(self) -> int:
        if self.byte_size > 0:
            return self.byte_size 
        else:
            value = int(self.to_unsigned())
            if value < 2**7:
                return 1
            elif value < 2 ** (7 * 9):
